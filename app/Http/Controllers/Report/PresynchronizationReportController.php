...
use App\Services\Report\Strategies\PresynchronizationReportStrategy;
use Barryvdh\DomPDF\Facade\Pdf;

class PresynchronizationReportController extends Controller
{
    public function __construct(
        private readonly ReportRunner $runner,
        private readonly PresynchronizationReportStrategy $strategy,
    ) {}
    ...
    public function export(ReportFilterRequest $request)
    {
        ...
        $pdf = Pdf::loadView('reports.presynchronization', compact('data', 'generatedAt'));
        ...
        // Descarga el PDF
        $filename = 'informe-presincronizacion-' . now()->format('Y-m-d') . '.pdf';
