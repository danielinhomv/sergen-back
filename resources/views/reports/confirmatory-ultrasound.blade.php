...
<head>
    <meta charset="UTF-8">
    <title>Informe de Confirmación de Preñez</title>
    <style>
        ...
        /** Heat Quality Indicators (Small dots/text) **/
        .hq-text {
            font-weight: bold;
            font-size: 9px;
        }
...
<body>
    <div class="header-container clearfix">
        <div class="header-left">
            ...
            <h3>Informe de Ecografía (Confirmación de Preñez)</h3>
        </div>
    ...
    <div class="info-box">
        <table class="filters-grid">
            <tr>
                ...
                <td><span class="label">Cobertura:</span><br> {{ number_format($summary['cobertura_pct'], 1) }}%
                    ({{ $summary['total_records'] }}/{{ $summary['hato_objetivo'] }})</td>
            </tr>
        </table>
    </div>
    ...
    <div class="kpi-container clearfix">
        <div class="kpi-card kpi-neutral">
            <span class="label">Total Diagnósticos</span>
            <span class="kpi-value">{{ number_format($summary['total_ecografias']) }}</span>
            <span class="kpi-sub">Animales revisados</span>
        </div>

        <div class="kpi-card kpi-success">
            <span class="label">PREÑEZ CONFIRMADA</span>
        ...
        <div class="kpi-card kpi-warning">
            <span class="label">Descartes / Refugos</span>
            <span class="kpi-value">{{ $summary['discart']['count'] + $summary['refugos']['count'] }}</span>
            <span class="kpi-sub">{{ number_format($summary['discart']['pct'] + $summary['refugos']['pct'], 1) }}% del
                total</span>
        </div>
    </div>
    ...
    <div class="clearfix" style="margin-bottom: 15px;">
                ...
                <tbody>
                    @forelse($data['distributions']['pregnancy_by_bull'] as $bull)
                        <tr>
                            ...
                            <td>{{ $bull['pregnant'] }}</td>
                            <td class="text-bold">
                                @if ($bull['pregnancy_rate_pct'] >= 50)
                                    <span
                                        style="color:#27ae60">{{ number_format($bull['pregnancy_rate_pct'], 1) }}%</span>
                                @else
                                    <span
        ...
        <div class="col-2">
            <h3>Efectividad según Celo (Insem.)</h3>
            <table>
                <thead>
                    <tr>
    ...
    <div style="page-break-before: auto;">
        <h2>Resultado Individual de Ecografía</h2>
        <table>
            <thead>
                <tr>
                    ...
                    <th style="width: 10%">Fecha Eco</th>
                    <th style="width: 20%">Datos Inseminación</th>
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            @elseif($item['status'] === 'empty')
                                <span class="badge bg-empty">VACÍA</span>
                            @elseif($item['status'] === 'discart' || $item['status'] === 'refugo')
                                <span class="badge bg-discart">DESCARTE</span>
                            @else
                        ...
                        <td class="text-left">
                            ...
                            <span class="small-text">Fecha:
                                {{ \Carbon\Carbon::parse($item['insemination_date'])->format('d/m/y') }}</span>
                        </td>
                        <td>
                            @php
    ...
    <div class="footer">
        CONFIDENCIAL | Sergen Empresa Ganadera | Reporte de Preñez | Pág. <span class="pagenum"></span>
    </div>
